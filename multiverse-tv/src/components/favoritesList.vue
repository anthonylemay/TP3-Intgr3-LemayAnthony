<template>
  <section class="minHeight">
    ...
    <div class="grid container" v-if="personnagesFavoris.length > 0">
      <cartePersonnage
        v-for="character in personnagesFavoris"
        :key="character.id"
        :personnage="character"
        @ouvrirPopUp="ouvrirZoomModal"
      />
      <ficheZoom
        :personnage="targetPersonnage"
        :isVisible="ouvrirPopUp"
        @update:isVisible="togglePopUpVisibility"
      />
    </div>
    <div class="flex flexCol" v-else>
      <p>
        Aucun personnage favoris d'ajouté pour le moment. Continuez à naviger!
      </p>
      <div class="flex btnLine">
        <RouterLink :to="{ name: 'personnages' }" class="btnNav"
...
const { personnagesFavoris, fetchPersonnagesFavoris } =
  useFetchPersonnagesFavoris();
